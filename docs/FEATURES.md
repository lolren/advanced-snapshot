...
| Video capture | Inherited recording flow, timer and duration indicator; shutter remains disabled until `video-done` and invalid empty outputs are rejected before gallery insertion | Working encoder/muxer and stable preview | Playable file, monotonic duration, clean stop, no duplicate stop request during finalization |
