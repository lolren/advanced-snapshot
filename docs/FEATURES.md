...
| --- | --- | --- | --- |
| Photo capture | Inherited Snapshot flow with full-frame mode selection; the OnePlus raw-still path reapplies tap/manual focus or triggers a fresh centre-weighted autofocus scan when neither is selected | A negotiated RGB/YUV still stream plus the standard AF controls on supported rear sensors | Decodable saved image at selected caps, with the focus operation applied to the stream that supplies the JPEG |
...
The installed OnePlus 6T libcamera r30/PipeWire r8 stack and r32 Advanced
Snapshot package pass the
...
Saturation, Contrast, Sharpness, Exposure, automatic/manual white balance, a
writable 3×3 colour matrix and, on rear sensors, focus—and stores them under a
stable sensor identity in GSettings. The OnePlus development stack exposes the
